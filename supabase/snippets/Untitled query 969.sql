...
CREATE TABLE products (
  ...
  name TEXT NOT NULL,
  currency TEXT[] DEFAULT ARRAY['RSD', 'EUR']::text[],
...
-- 5. Заливаем продукты (Ошибка типов ИСПРАВЛЕНА)
INSERT INTO products (bank_id, category, name, currency)
SELECT id, 'current_account', name || ': Личный счет (RSD/EUR)', ARRAY['RSD', 'EUR']::text[] FROM banks
UNION ALL
SELECT id, 'business_account', name || ': Счет для ИП (Preduzetnik)', ARRAY['RSD', 'EUR']::text[] FROM banks
UNION ALL
SELECT id, 'savings', name || ': Сберегательный вклад', ARRAY['EUR']::text[] FROM banks;
